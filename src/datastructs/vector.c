...
void ratr0_vector_shutdown(void)
{
    for (int i = 0; i < num_vectors; i++) {
        engine->memory_system->free_block(vectors[i].handle);
    }
}
...
struct Ratr0Vector *ratr0_new_vector(UINT16 initial_capacity, UINT16 resize_by)
{
    ...
    result->handle = engine->memory_system->allocate_block(RATR0_MEM_DEFAULT,
                                                           initial_capacity * sizeof(void *));
    result->elements = (void **) engine->memory_system->block_address(result->handle);
...
void ratr0_vector_append(struct Ratr0Vector *vec, void *elem)
{
    if (vec->num_elements == vec->capacity) {
        ...
        Ratr0MemHandle new_handle = engine->memory_system->allocate_block(RATR0_MEM_DEFAULT,
                                                                          new_capacity * sizeof(void *));
        void **new_elems = engine->memory_system->block_address(new_handle);
        for (int i = 0; i < vec->num_elements; i++) {
            new_elems[i] = vec->elements[i];
        }
        // Free the old system and assign the new one
        engine->memory_system->free_block(vec->handle);
